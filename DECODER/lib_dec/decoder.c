...
int main()
{
	char Inputfile[60]="J:\\SAMPLE\\EVS_SAMPLE\\16kHz\\sample0000.dat";
	char Outputfile[60]="J:\\SAMPLE\\EVS_SAMPLE\\16kHz\\c_sample0000.pcm";
	decoder(Inputfile,Outputfile);
	return 0;
}

int decoder(char Inputfile[] ,char Outputfile[])
{
	...
	argv[1]="-VOIP";//
	argv[2]="16";//
	argv[3]=Inputfile;//输入文件
	argv[4]=Outputfile;//输出文件
    ...
    io_ini_dec_fx( argc, argv, &f_stream, &f_synth,&quietMode,&noDelayCmp,st_fx,
#ifdef SUPPORT_JBM_TRACEFILE
                   &jbmTraceFileName,
#endif
